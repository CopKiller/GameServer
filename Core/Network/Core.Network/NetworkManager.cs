...
public class NetworkManager : INetworkManager
{
    ...
    public ICustomNetPeer ConnectToServer(string address, int port, string key)
        => new CustomNetPeer(_networkManager.Connect(address, port, key));

    public ICustomNetPeer ConnectToServer(IPEndPoint target, string key)
        => new CustomNetPeer(_networkManager.Connect(target, key));
    ...
    public int GetPeersCount(CustomConnectionState peerState)
        => _networkManager.GetPeersCount(Extensions.ConvertToConnectionState(peerState));
    ...
    public ICustomNetPeer GetFirstPeer()
        => new CustomNetPeer(_networkManager.FirstPeer);
    ...
    public void DisconnectPeerForce(ICustomNetPeer peer)
    {
        if (peer is not CustomNetPeer customNetPeer){
            throw new ArgumentException("Peer is not CustomNetPeer");
        }
