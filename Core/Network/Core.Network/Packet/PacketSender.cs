...
public class PacketSender(NetPacketProcessor packetProcessor, IConnectionManager manager) : IPacketSender
{
        ...
        CustomDeliveryMethod deliveryMethod = CustomDeliveryMethod.ReliableOrdered) where T : ICustomSerializable
    {
        ...
        packetProcessor.SendNetSerializable(liteNetPeer.Peer, ref adapter,
            Extensions.ConvertToLiteDeliveryMethod(deliveryMethod));
    }
        ...
        CustomDeliveryMethod deliveryMethod = CustomDeliveryMethod.ReliableOrdered) where TPacket : class, new()
    {
        ...
        foreach (var peer in allPeers) SendPacket(peer, packet, deliveryMethod);
    }
        ...
        CustomDeliveryMethod deliveryMethod = CustomDeliveryMethod.ReliableOrdered) where T : ICustomSerializable
    {
        ...
        foreach (var peer in allPeers) SendPacket(peer, adapter, deliveryMethod);
    }
    ...
    public void SendPacketToAll(byte[] data, CustomDeliveryMethod deliveryMethod = CustomDeliveryMethod.ReliableOrdered)
    {
        ...
        foreach (var peer in allPeers) peer.Send(data, deliveryMethod);
    }
}